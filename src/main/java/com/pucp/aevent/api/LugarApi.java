...
public class LugarApi {
	...
	@Secured({"ROLE_ADMIN"})
	@GetMapping(path = "/lugar/activasPaginadas", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseObject> consultarActivasPaginadas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<Lugar> lista = this.lugarService.findByEnabled(true,PageRequest.of(page.getPaginaFront(), page.getRegistros()));
			response.setResultado(lista);
			response.setPaginacion(lugarService.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
	...
	@Secured({"ROLE_ADMIN"})
	@GetMapping(path = "/lugar/activas", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseObject> consultarActivas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<Lugar> lista = this.lugarService.findByEnabled(true);
			response.setResultado(lista);
			response.setPaginacion(lugarService.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
	...
	@Secured({"ROLE_ADMIN"})
	@GetMapping(path = "/lugar/inactivasPaginadas", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseObject> consultarInctivasPaginadas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<Lugar> lista = this.lugarService.findByEnabled(false,PageRequest.of(page.getPaginaFront(), page.getRegistros()));
			response.setResultado(lista);
			response.setPaginacion(lugarService.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
	...
	@Secured({"ROLE_ADMIN"})
	@GetMapping(path = "/lugar/inactivas", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseObject> consultarInactivas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<Lugar> lista = this.lugarService.findByEnabled(false);
			response.setResultado(lista);
			response.setPaginacion(this.lugarService.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
		} catch(BadRequest e) {
			//response.setError(this.service.getError());
	...
	@Secured({"ROLE_ADMIN"})
	@GetMapping(path = "/lugar/filtroNombre", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseObject> consultarFiltradoNombre(String descripcion,boolean enabled,PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<Lugar> lista = this.lugarService.findByDescripcionContainingAndEnabled(descripcion, enabled, PageRequest.of(page.getPaginaFront(), page.getRegistros()));
			response.setResultado(lista);
			response.setPaginacion(lugarService.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
