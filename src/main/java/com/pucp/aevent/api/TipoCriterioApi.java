...
public class TipoCriterioApi {
	...
	public ResponseEntity<ResponseObject> consultarActivasPaginadas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<TipoCriterio> lista = this.service.findByEnabled(true,PageRequest.of(page.getPaginaFront(), page.getRegistros()));
			response.setResultado(lista);
			response.setPaginacion(service.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
	...
	public ResponseEntity<ResponseObject> consultarActivas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<TipoCriterio> lista = this.service.findByEnabled(true);
			response.setResultado(lista);
			response.setPaginacion(service.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
	...
	public ResponseEntity<ResponseObject> consultarInctivasPaginadas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<TipoCriterio> lista = this.service.findByEnabled(false,PageRequest.of(page.getPaginaFront(), page.getRegistros()));
			response.setResultado(lista);
			response.setPaginacion(service.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
	...
	public ResponseEntity<ResponseObject> consultarInactivas(PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<TipoCriterio> lista = this.service.findByEnabled(false);
			response.setResultado(lista);
	...
	@Secured({"ROLE_ADMIN"})
	@GetMapping(path = "/tipocriterios/filtroNombre", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseObject> consultarFiltradoNombre(String descripcion,boolean enabled,PaginaRequest page) {
		ResponseObject response = new ResponseObject();
		try {
			List<TipoCriterio> lista = this.service.findByDescripcionContainingAndEnabled(descripcion, enabled, PageRequest.of(page.getPaginaFront(), page.getRegistros()));
			response.setResultado(lista);
			response.setPaginacion(service.getPaginacion());
			response.setEstado(Estado.OK);
			return new ResponseEntity<ResponseObject>(response, HttpStatus.OK);
